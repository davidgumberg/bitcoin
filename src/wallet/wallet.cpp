...
DBErrors CWallet::PopulateWalletFromDB(bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (nLoadWalletRet != DBErrors::LOAD_OK) {
        auto dberror_string = DBErrorsToString(nLoadWalletRet, wallet_file);
        Assume(dberror_string.has_value());

        if (nLoadWalletRet == DBErrors::NONCRITICAL_ERROR || nLoadWalletRet == DBErrors::NEED_RESCAN) {
            warnings.push_back(dberror_string.value());
        } else {
            error = dberror_string.value();
        }
    }
