...
int Sock::GetOSBytesQueued() const
{
    int bytes_queued{0};
#if (defined(__linux__) || defined(__FreeBSD__))
    if (ioctl(m_socket, TIOCOUTQ, &bytes_queued)) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Error, "ioctl(TIOCOUTQ): %s\n", NetworkErrorString(errno));
        return 0;
    }
#elif defined(__NetBSD__) // https://man.netbsd.org/ioctl.2
    if (ioctl(m_socket, FIONWRITE, &bytes_queued)) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Error, "ioctl(FIONWRITE): %s\n", NetworkErrorString(errno));
        return 0;
    }
    ...
    if (GetSockOpt(SOL_SOCKET, SO_NWRITE, &bytes_queued, &result_len)) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Error, "getsockopt(SO_NWRITE): %s\n", NetworkErrorString(errno));
        return 0;
    }
#elif defined(__OpenBSD__)
    TCPInfo i{m_socket};
    if (!i.m_valid) {
        LogPrintLevel(BCLog::NET, BCLog::Level::Error, "Acquiring TCP info: %s\n", NetworkErrorString(errno));
        return 0;
    }

    // Index of Next byte to send - index of oldest unacked byte.
    bytes_queued = i.m_tcp_info.tcpi_snd_nxt - i.m_tcp_info.tcpi_snd_una;
    ...
    bytes_queued = i.m_tcp_info.BytesInFlight;
#endif
