...
bool PeerManagerImpl::SendMessages(CNode& node)
{
...

    {
        ...
        //
        std::vector<CInv> vGetData;
        if ((m_chainman.IsInitialBlockDownload() || node.IsManualConn()) && CanServeBlocks(peer) && ((sync_blocks_and_headers_from_peer && !IsLimitedPeer(peer)) || !m_chainman.IsInitialBlockDownload()) && state.vBlocksInFlight.size() < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
