...
static void BlockEncodingBench(benchmark::Bench& bench, size_t n_pool, size_t n_extra, size_t n_random_in_block, size_t n_pool_in_block = 0, size_t n_extra_in_block = 0, bool shuffle = true)
{
    ...
    for (size_t i = 1; i <= n_pool_in_block; i++) {
        // Insert the highest fee-rate first, reverse order.
        refs_for_block.push_back(mempool_refs[n_pool - i]);
    }
    for (size_t i = 1; i <= n_extra_in_block; i++) {
        // Insert the highest fee-rate first, reverse order.
        refs_for_block.push_back(extra_refs[n_extra - i]);
    }
