...
static leveldb::Options GetOptions(size_t nCacheSize)
{
    ...
    options.max_file_size = std::max(options.max_file_size, LEVELDBWRAPPER_MAX_FILE_SIZE);
    SetMaxOpenFiles(&options);
    return options;
}

struct LevelDBBatch::WriteBatchImpl {
    leveldb::WriteBatch batch;
};

LevelDBBatch::LevelDBBatch(const DBWrapperBase& _parent)
    : DBBatchBase(_parent),
      m_impl_batch{std::make_unique<LevelDBBatch::WriteBatchImpl>()}
{
    Clear();
};

LevelDBBatch::~LevelDBBatch() = default;

void LevelDBBatch::Clear()
{
    m_impl_batch->batch.Clear();
}

void LevelDBBatch::WriteImpl(std::span<const std::byte> key, DataStream& value)
{
...
void LevelDBBatch::EraseImpl(std::span<const std::byte> key)
{
    leveldb::Slice slKey(CharCast(key.data()), key.size());
    m_impl_batch->batch.Delete(slKey);
}

size_t LevelDBBatch::ApproximateSize() const
{
...
LevelDBWrapper::LevelDBWrapper(const DBParams& params)
...
void LevelDBWrapper::WriteBatch(DBBatchBase& _batch, bool fSync)
{
    LevelDBBatch& batch = static_cast<LevelDBBatch&>(_batch);
...
size_t LevelDBWrapper::DynamicMemoryUsage() const
{
    std::string memory;
...
std::optional<std::string> LevelDBWrapper::ReadImpl(std::span<const std::byte> key) const
{
    leveldb::Slice slKey(CharCast(key.data()), key.size());
    std::string strValue;
...
bool LevelDBWrapper::ExistsImpl(std::span<const std::byte> key) const
{
    leveldb::Slice slKey(CharCast(key.data()), key.size());
...
size_t LevelDBWrapper::EstimateSizeImpl(std::span<const std::byte> key1, std::span<const std::byte> key2) const
{
...
bool LevelDBWrapper::DestroyDB(const std::string& path_str)
{
    return leveldb::DestroyDB(path_str, {}).ok();
}

struct LevelDBIterator::IteratorImpl {
    const std::unique_ptr<leveldb::Iterator> iter;

    explicit IteratorImpl(leveldb::Iterator* _iter) : iter{_iter} {}
};

LevelDBIterator::LevelDBIterator(const DBWrapperBase& _parent, std::unique_ptr<IteratorImpl> _piter)
    : DBIteratorBase(_parent),
      m_impl_iter(std::move(_piter)) {}

DBIteratorBase* LevelDBWrapper::NewIterator()
{
    return new LevelDBIterator{*this, std::make_unique<LevelDBIterator::IteratorImpl>(DBContext().pdb->NewIterator(DBContext().iteroptions))};
}

void LevelDBIterator::SeekImpl(std::span<const std::byte> key)
{
    leveldb::Slice slKey(CharCast(key.data()), key.size());
    m_impl_iter->iter->Seek(slKey);
}

std::span<const std::byte> LevelDBIterator::GetKeyImpl() const
{
    return MakeByteSpan(m_impl_iter->iter->key());
}

std::span<const std::byte> LevelDBIterator::GetValueImpl() const
{
    return MakeByteSpan(m_impl_iter->iter->value());
}

LevelDBIterator::~LevelDBIterator() = default;
bool LevelDBIterator::Valid() const { return m_impl_iter->iter->Valid(); }
void LevelDBIterator::SeekToFirst() { m_impl_iter->iter->SeekToFirst(); }
void LevelDBIterator::Next() { m_impl_iter->iter->Next(); }
