...
BOOST_AUTO_TEST_CASE(dbwrapper)
{
    ...
    for (const bool obfuscate : {false, true}) {
        ...
        LMDBWrapper dbw({.path = ph, .cache_bytes = 1 << 20, .memory_only = true, .wipe_data = false, .obfuscate = obfuscate});
        uint8_t key{'k'};
...
BOOST_AUTO_TEST_CASE(dbwrapper_basic_data)
{
    ...
    for (bool obfuscate : {false, true}) {
        ...
        LMDBWrapper dbw({.path = ph, .cache_bytes = 1 << 20, .memory_only = false, .wipe_data = true, .obfuscate = obfuscate});
...
BOOST_AUTO_TEST_CASE(existing_data_no_obfuscate)
{
    ...
    std::unique_ptr<LMDBWrapper> dbw = std::make_unique<LMDBWrapper>(DBParams{.path = ph, .cache_bytes = 1 << 10, .memory_only = false, .wipe_data = false, .obfuscate = false});
    uint8_t key{'k'};
    uint256 in = m_rng.rand256();
    uint256 res;
    ...
    LMDBWrapper odbw({.path = ph, .cache_bytes = 1 << 10, .memory_only = false, .wipe_data = false, .obfuscate = true});
...
BOOST_AUTO_TEST_CASE(existing_data_reindex)
{
    ...
    LMDBWrapper odbw({.path = ph, .cache_bytes = 1 << 10, .memory_only = false, .wipe_data = true, .obfuscate = true});
...
BOOST_AUTO_TEST_CASE(iterator_ordering)
{
    ...
    LMDBWrapper dbw({.path = ph, .cache_bytes = 1 << 20, .memory_only = true, .wipe_data = false, .obfuscate = false});
...
BOOST_AUTO_TEST_CASE(unicodepath)
{
    ...
    LMDBWrapper dbw({.path = ph, .cache_bytes = 1 << 20});

    fs::path lockPath = ph / "lmdb.lck";
