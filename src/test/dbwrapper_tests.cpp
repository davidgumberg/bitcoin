...
BOOST_AUTO_TEST_CASE(dbwrapper)
{
    ...
    for (const bool obfuscate : {false, true}) {
        ...
        // Write values
        {
            DBWrapper dbw{{.path = path, .cache_bytes = CACHE_SIZE, .wipe_data = true, .obfuscate = obfuscate}};
        ...
        {
            DBWrapper dbw{{.path = path, .cache_bytes = CACHE_SIZE, .obfuscate = false}};
            BOOST_CHECK_EQUAL(obfuscation, dbw.GetObfuscation());
        }

        // Read back the values
        {
            DBWrapper dbw{{.path = path, .cache_bytes = CACHE_SIZE, .obfuscate = obfuscate}};
...
BOOST_AUTO_TEST_CASE(dbwrapper_basic_data)
{
    ...
    for (bool obfuscate : {false, true}) {
        ...
        DBWrapper dbw({.path = ph, .cache_bytes = 1 << 20, .memory_only = false, .wipe_data = true, .obfuscate = obfuscate});
...
BOOST_AUTO_TEST_CASE(dbwrapper_batch)
{
    ...
    for (const bool obfuscate : {false, true}) {
        ...
        DBWrapper dbw({.path = ph, .cache_bytes = 1 << 20, .memory_only = true, .wipe_data = false, .obfuscate = obfuscate});
...
BOOST_AUTO_TEST_CASE(dbwrapper_iterator)
{
    ...
    for (const bool obfuscate : {false, true}) {
        ...
        std::unique_ptr<DBIteratorBase> it(const_cast<DBWrapper&>(dbw).NewIterator());
...
BOOST_AUTO_TEST_CASE(existing_data_no_obfuscate)
{
    ...
    std::unique_ptr<DBWrapper> dbw = std::make_unique<DBWrapper>(DBParams{.path = ph, .cache_bytes = 1 << 10, .memory_only = false, .wipe_data = false, .obfuscate = false});
    uint8_t key{'k'};
    uint256 in = m_rng.rand256();
    uint256 res;
    ...
    DBWrapper odbw({.path = ph, .cache_bytes = 1 << 10, .memory_only = false, .wipe_data = false, .obfuscate = true});
...
BOOST_AUTO_TEST_CASE(existing_data_reindex)
{
    ...
    DBWrapper odbw({.path = ph, .cache_bytes = 1 << 10, .memory_only = false, .wipe_data = true, .obfuscate = true});
...
BOOST_AUTO_TEST_CASE(iterator_ordering)
{
    ...
    DBWrapper dbw({.path = ph, .cache_bytes = 1 << 20, .memory_only = true, .wipe_data = false, .obfuscate = false});
...
BOOST_AUTO_TEST_CASE(unicodepath)
{
    ...
    DBWrapper dbw({.path = ph, .cache_bytes = 1 << 20});
