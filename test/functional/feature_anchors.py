...
class AnchorsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.debug("Stop node")
        with self.nodes[0].assert_debug_log(["DumpAnchors(const fs::path &, const std::vector<CAddress> &): Flush 1 outbound block-relay-only peer addresses to anchors.dat"]):
            self.stop_node(0)
